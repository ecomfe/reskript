import {deprecatedWarn, logger} from '@reskript/core';
...
export const warnDeprecatedInProjectSettings = (settings: DeepPartial<ProjectSettings>): void => {
    if (settings.build?.script?.defaultImportOptimization !== undefined) {
        ...
        deprecatedWarn('build.script.defaultImportOptimization is deprecated, use build.uses instead');
    ...
    if (settings.play?.wrapper !== undefined) {
        ...
        deprecatedWarn('play.wrapper is deprecated, use standalone play configuration instead');
    ...
    if (settings.play?.injectResources !== undefined) {
        const documentLink = 'https://ecomfe.github.io/reskript/docs/advanced/debug-component#自定义调试预览的布局';
        deprecatedWarn('play.injectResources is deprecated, use defaultGlobalSetup instead');
