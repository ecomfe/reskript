...
async function dumpAsModule(json, destination) {
    ...
    const output = await prettier.format(moduleCode, prettyOptions);
...
await dumpAsModule(jestPresetConfig('react'), path.join(destination, 'jest-react.js'));
await dumpAsModule(jestPresetConfig('node'), path.join(destination, 'jest-node.js'));
